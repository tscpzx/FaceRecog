...
import com.cjw.library.utils.DrawableUtil;
...
public class ShapeImageView extends AppCompatImageView {
	...
	public void setImageResource(int resId) {
		...
		mBitmap = DrawableUtil.getBitmapFromDrawable(getDrawable());
		setupBitmapShader();
	}
	...
	public void setImageDrawable(Drawable drawable) {
		...
		mBitmap = DrawableUtil.getBitmapFromDrawable(drawable);
		setupBitmapShader();
	}
	...
	public void onDraw(Canvas canvas) {
		...
		if (mBitmap != null) {
			if (mShape == SHAPE_CIRCLE) {
				canvas.drawCircle(mViewRect.right / 2, mViewRect.bottom / 2, Math.min(mViewRect.right, mViewRect.bottom) / 2, mBitmapPaint);
				...
				mPath.reset();
				mPath.addRoundRect(mViewRect, new float[]{mRoundRadiusLeftTop, mRoundRadiusLeftTop, mRoundRadiusRightTop, mRoundRadiusRightTop, mRoundRadiusRightBottom, mRoundRadiusRightBottom, mRoundRadiusLeftBottom, mRoundRadiusLeftBottom,}, Path.Direction.CW);
		...
		if (mBorderSize > 0) { // 绘制边框
			if (mShape == SHAPE_CIRCLE) {
				canvas.drawCircle(mViewRect.right / 2, mViewRect.bottom / 2, Math.min(mViewRect.right, mViewRect.bottom) / 2 - mBorderSize / 2, mBorderPaint);
			} else if (mShape == SHAPE_OVAL) {
				canvas.drawOval(mBorderRect, mBorderPaint);
			} else {
				//                canvas.drawRoundRect(mBorderRect, mRoundRadius, mRoundRadius, mBorderPaint);
				mPath.reset();
				mPath.addRoundRect(mBorderRect, new float[]{mRoundRadiusLeftTop, mRoundRadiusLeftTop, mRoundRadiusRightTop, mRoundRadiusRightTop, mRoundRadiusRightBottom, mRoundRadiusRightBottom, mRoundRadiusLeftBottom, mRoundRadiusLeftBottom,}, Path.Direction.CW);
